...
from ..models import *
from ..serializers import StationCSerilizers,GetStationCPostSerializers
...
class StationCDetailsCreate(generics.GenericAPIView):
    serializer_class = StationCSerilizers
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if StationCModel.objects.filter(HCID=hcid, InfoseekId=infoseekid):
                raise Exception("This HCID already exits with InfoseekId")
            else:
                data = request.data
                check_VisionCorrected(request)
                field_lists = {
                    'Vision_Corrected_Yes' : Vision_Corrected,
                    }
                ...
                station_instance = StationNamesModel.objects.get(id=3)
                serializer.validated_data['StationID'] = station_instance
                user = serializer.save()
                response = GenericResponse("Message", "Result", "Status", "HasError")
                response.Message = "Successful"
                response.Result = GetStationCPostSerializers(user).data
                response.Status = 200
